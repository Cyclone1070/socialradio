...
describe('AuthService', () => {
  ...
  describe('register', () => {
    it('should hash password and create user', async () => {
      ...
      const result = await (service as any).register(registerDto);

      expect(bcrypt.hash).toHaveBeenCalledWith(registerDto.password, 10);
      expect(mockUserService.create).toHaveBeenCalledWith(registerDto, hashedPassword);
  ...
  describe('login', () => {
    it('should return access token if credentials are valid', async () => {
      ...
      const user = { id: 'uuid', email: 'test@example.com', passwordHash: 'hash' };

      mockUserService.findByEmail.mockResolvedValue(user);
      (bcrypt.compare as jest.Mock).mockResolvedValue(true);
      mockJwtService.sign.mockReturnValue('signed_jwt');

      const result = await (service as any).login(loginDto);
    ...
    it('should throw UnauthorizedException if user is not found', async () => {
      ...
      await expect((service as any).login(loginDto)).rejects.toThrow(UnauthorizedException);
      expect(mockUserService.findByEmail).toHaveBeenCalledWith(loginDto.email);
