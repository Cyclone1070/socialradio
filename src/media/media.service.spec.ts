...
describe('MediaService', () => {
  ...
  describe('getRandomMusic', () => {
    it('should return a random music track as a Segment', async () => {
      const tracks = [
        { id: '1', title: 'Song 1', artist: 'Art 1', filePath: 'song1.mp3', durationSeconds: 180 },
        { id: '2', title: 'Song 2', artist: 'Art 2', filePath: 'song2.mp3', durationSeconds: 240 },
      ];
      mockMusicRepo.find.mockResolvedValue(tracks);

      const result = await (service as any).getRandomMusic();
    ...
    it('should throw NotFoundException if no music tracks exist', async () => {
      ...
      await expect((service as any).getRandomMusic()).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('getRandomAd', () => {
    it('should return a random ad track as a Segment', async () => {
      const ads = [
        { id: '1', advertiser: 'Brand 1', filePath: 'ad1.mp3', durationSeconds: 30 },
      ];
      mockAdRepo.find.mockResolvedValue(ads);

      const result = await (service as any).getRandomAd();
    ...
    it('should throw NotFoundException if no ads exist', async () => {
      ...
      await expect((service as any).getRandomAd()).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('getRandomJingle', () => {
    it('should return a random jingle track as a Segment', async () => {
      const jingles = [
        { id: '1', name: 'Jingle 1', filePath: 'jingle1.mp3', durationSeconds: 5 },
      ];
      mockJingleRepo.find.mockResolvedValue(jingles);

      const result = await (service as any).getRandomJingle();
    ...
    it('should throw NotFoundException if no jingles exist', async () => {
      ...
      await expect((service as any).getRandomJingle()).rejects.toThrow(NotFoundException);
    });
  });
});
